...
@Component({
  ...
  styleUrls: ['./financialstatement.component.scss']
})
...
export class financialstatementComponent implements OnInit {
  ...
  constructor(private commonService: CommonService,
    ...
    public dialogRef: MatDialogRef<financialstatementComponent>,
    // @Optional() is used to prevent error if no data is passed
    @Optional() @Inject(MAT_DIALOG_DATA) public data: any) {

    this.modelFormData = this.formBuilder.group({
      structureKey: ['', [Validators.required, Validators.minLength(2), Validators.maxLength(4)]],
      structureName: ['', [Validators.required, Validators.minLength(2)]],
    });

    this.formData = { ...data };
    if (!this.commonService.checkNullOrUndefined(this.formData.item)) {
      this.modelFormData.patchValue(this.formData.item);
      this.modelFormData.controls['structureKey'].disable();
    }
  ...
  save() {
    ...
    this.modelFormData.controls['structureKey'].enable();
